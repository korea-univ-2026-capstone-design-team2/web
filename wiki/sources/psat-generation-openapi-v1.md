...
- 문제 생성 진행 확인은 `GET /question-generations/{generationId}/events` SSE로 제공된다.
...
- 최신 Swagger 기준 `/questions/papers`, `/questions/reviews`는 `POST`이다.
...
- 문제 생성 화면은 `POST /question-generations` 응답의 `generationId`로 SSE 이벤트를 구독한다.
